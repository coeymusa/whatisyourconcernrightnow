...
// Edge-cache the GET so each region hits Supabase at most once per 10s.
// stale-while-revalidate keeps the response warm for another 30s while the
// next read refreshes in the background.
const PUBLIC_CACHE = {
...
export async function POST(req: Request) {
  ...
  const mod = moderate(text, { age, countryCode: country });
  if (mod.kind === "block") {
